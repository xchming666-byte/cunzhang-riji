...
function processInjuries(){
  // 1. 一定概率新增轻伤
  if(population>0&&injuries.length<population&&Math.random()<INJURE_CHANCE){
  ...
  if(hasClinic()&&injuries.length){
    ...
    for(const{o} of order){
      if(healed>=CLINIC_HEAL_PER_DAY)break;
...
// ===================== 存档系统 =====================
function getSaveData(){return{resources:{...RESOURCES},population,popMax,satisfaction,villageLevel,day,gameTime,buildings:buildings.map(b=>({type:b.type,workers:b.workers,level:b.level,guards:b.guards||0,scholars:b.scholars||0,index:b.index})),injuries:injuries.map(x=>({sev:x.sev,days:x.days})),health,research,officials,stats:{...stats},unlockedAchievements:[...unlockedAchievements],gameWon,saveTime:new Date().toLocaleString('zh-CN',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})}}
