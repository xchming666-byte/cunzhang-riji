...
// 建筑定义
const BT={
  farm:{n:'农田',c:'🌾',img:'img/farm.svg',cost:{wood:5},maxW:3,prod:{food:2},cons:{water:1},minLv:1,unlocked:true},
  lumber:{n:'木材坊',c:'🪵',img:'img/lumber.svg',cost:{wood:3},maxW:2,prod:{wood:0.5},cons:{water:1},minLv:1,unlocked:true},
  well:{n:'水井',c:'💧',img:'img/well.svg',cost:{wood:5,gold:20},maxW:3,prod:{water:2},minLv:1,unlocked:true},
  house:{n:'民居',c:'🏠',img:'img/house.svg',cost:{wood:8,gold:30},maxW:0,popB:3,minLv:1,unlocked:true},
  quarry:{n:'采石场',c:'⛏️',img:'img/quarry.svg',cost:{wood:10,gold:50},maxW:2,prod:{stone:1},minLv:2,unlocked:false},
  market:{n:'集市',c:'🏪',img:'img/market.svg',cost:{wood:15,stone:5},maxW:2,prod:{gold:5},minLv:2,unlocked:false},
  clinic:{n:'医馆',c:'🏥',img:'img/clinic.svg',cost:{stone:10,gold:80},maxW:1,minLv:2,unlocked:false},
  school:{n:'学堂',c:'📚',img:'img/school.svg',cost:{stone:15,gold:120},maxW:3,minLv:2,unlocked:false},
  teahouse:{n:'茶馆',c:'🍵',img:'img/teahouse.svg',cost:{stone:15,gold:150},maxW:1,minLv:3,unlocked:false},
  wall:{n:'城墙',c:'🏰',img:'img/wall.svg',cost:{stone:50},maxW:0,minLv:3,unlocked:false},
  temple:{n:'祠堂',c:'⛩️',img:'img/temple.svg',cost:{stone:30,gold:200},maxW:1,minLv:3,unlocked:false},
...
// ===================== 满意度动态平衡（解决"几乎总是100%"）=====================
// satB 建筑不再"建造瞬间封顶加分"，改为每日持续供给；同时引入随村庄规模增长的"村民怨气"每日消耗。
// 两者在 tick 每日结算里对冲，满意度不再硬锁 100，需要持续经营维持。
const SAT_DRAIN_POP_LV12=15;  // 早期(Lv1~2)每多少人口才产生 1 点怨气（人口<15 时不消耗，保护升级门槛）
const SAT_DRAIN_POP=6;        // 中后期(Lv3+)每多少人口产生 1 点怨气
const SAT_DRAIN_LV_BONUS=2;   // 超过 Lv2 后，每高一级每日额外怨气
const SAT_SUPPLY_RATE=0.5;    // 每 1 点 satB 建筑每日供给系数（如医馆10→每日+5）
function satDrainPerDay(){
  if(villageLevel<=2) return Math.floor(population/SAT_DRAIN_POP_LV12);
  return Math.floor(population/SAT_DRAIN_POP)+(villageLevel-2)*SAT_DRAIN_LV_BONUS;
}
// 安抚建筑按等级差异化：satB 基础值（×SAT_SUPPLY_RATE=每日供给），升级显著增强安抚力（解决"升不升级一样"）
const SATB_LVL={clinic:[10,18,28,40,56],teahouse:[15,22,30,40,52],temple:[20,30,42,56,72],paifang:[15,20,28,36,48]};
...
const TECH={
  ...
  // 工业
  ind_efficient:{n:'高效伐木',c:'🪵',branch:'工业',cost:30,pre:[],ds:'木材坊产出 +25%'},
...
// ===================== 建筑操作 =====================
function placeBuilding(idx,cell){const bt=BT[selectedBuilding];if(villageLevel<bt.minLv)return;for(const[k,v]of Object.entries(bt.cost)){if(RESOURCES[k]<v){addLog(`建造${bt.n}失败：资源不足`,'bad');return}}for(const[k,v]of Object.entries(bt.cost))RESOURCES[k]-=v;const b={type:selectedBuilding,workers:0,level:1,guards:0,index:idx};if(bt.maxW>0&&bt.prod&&Object.keys(bt.prod).length&&getIdlePop()>=1)b.workers=1;gridData[idx]=b;buildings.push(b);renderCell(idx,cell);if(bt.popB)popMax+=bt.popB;stats.buildingsBuilt++;addLog(`建造了${bt.n}`,'good');flashResource('res-gold');selectedBuilding=null;document.querySelectorAll('.build-btn').forEach(b=>b.classList.remove('selected'));updateUI()}

function tryUpgradeBuilding(idx,cell){const b=gridData[idx];if(!b)return;const bt=BT[b.type];if(b.level>=5){addLog(`${bt.n}已是最高等级`,'event');return}const cost={};for(const[k,v]of Object.entries(bt.cost))cost[k]=Math.floor(v*(b.level+1)*.8);for(const[k,v]of Object.entries(cost)){if(RESOURCES[k]<v){addLog(`${bt.n}升级失败：资源不足`,'bad');return}}for(const[k,v]of Object.entries(cost))RESOURCES[k]-=v;b.level++;stats.upgradesDone++;const bi=buildings.findIndex(bb=>bb.index===idx);if(bi>=0)buildings[bi].level=b.level;if(bt.popB)popMax+=bt.popB;cell.querySelector('.level-badge').textContent=`Lv${b.level}`;addLog(`${bt.n}升级到Lv${b.level}${SATB_LVL[b.type]?'，安抚升至每日+'+(SATB_LVL[b.type][b.level-1]*SAT_SUPPLY_RATE):''}`,'good');updateUI()}
