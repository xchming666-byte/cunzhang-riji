...
let villageLevel=1,research=0,officials=0,day=1,gameTime=0;
...
const BT={
  ...
  mill:{n:'磨坊',c:'🏭',cost:{stone:20,wood:20},maxW:1,minLv:3,unlocked:false},
...
const EVENTS=[
  ...
  {type:'plague',w:6,minLv:1,t:'🦠 瘟疫蔓延',txt:'村庄爆发瘟疫，村民们生病了。',ch:[
    {t:'全力救治 (-100💰, -20😊)',a:()=>{if(RESOURCES.gold>=100){RESOURCES.gold-=100;satisfaction=Math.max(0,satisfaction-20);addLog('全力救治村民，金币-100','bad')}else{population=Math.max(1,population-2);satisfaction=Math.max(0,satisfaction-15);addLog('无力救治，2位村民病逝','bad')}}},
  ...
  {type:'taxman',w:5,minLv:3,t:'👑 朝廷来使',txt:'朝廷派来了税务官，要求缴纳税收。',ch:[
    {t:'乖乖交税 (-150💰,+10😊)',a:()=>{if(RESOURCES.gold>=150){RESOURCES.gold-=150;satisfaction=Math.min(100,satisfaction+10);addLog('交了税，朝廷很满意','good')}else{population=Math.max(1,population-1);addLog('没钱交税，1位村民被征去服劳役','bad')}}},
    {t:'贿赂官员 (-100💰)',a:()=>{if(RESOURCES.gold>=100){RESOURCES.gold-=100;addLog('税务官收了好处，睁一只眼闭一只眼','event')}else{population=Math.max(1,population-1);addLog('没钱交税，1位村民被征去服劳役','bad')}}}]}
];
...
const ACHIEVEMENTS=[
  ...
  {id:'research100',g:'治理',ic:'📚',nm:'学富五车',ds:'研究点达100',rw:{gold:400},ck:()=>research>=100},
...
function hasMill(){return buildings.some(b=>b.type==='mill')}
...
// ===================== 网格 =====================
function remapBuildings(oldC,oldR,newC,newR){const old=gridData.slice();gridData.length=0;for(let i=0;i<newC*newR;i++)gridData.push(null);for(let i=0;i<oldC*oldR;i++){if(!old[i])continue;const oc=i%oldC,or=Math.floor(i/oldC);let nc=Math.min(newC-1,Math.round(oc*newC/oldC)),nr=Math.min(newR-1,Math.round(or*newR/oldR));for(let d=0;d<20;d++){const dc=[0,1,-1,0,1,-1,1,-1][d%8],dr=[1,0,0,-1,-1,1,-1,1][d%8];if(!gridData[nr*newC+nc])break;nc=Math.max(0,Math.min(newC-1,Math.round(oc*newC/oldC)+dc));nr=Math.max(0,Math.min(newR-1,Math.round(or*newR/oldR)+dr))}const ni=nr*newC+nc;gridData[ni]=old[i];gridData[ni].index=ni}}
...
// ===================== 存档系统 =====================
function getSaveData(){return{resources:{...RESOURCES},population,popMax,satisfaction,villageLevel,day,gameTime,buildings:buildings.map(b=>({type:b.type,workers:b.workers,level:b.level,guards:b.guards||0,scholars:b.scholars||0,index:b.index})),health,research,officials,stats:{...stats},unlockedAchievements:[...unlockedAchievements],gameWon,saveTime:new Date().toLocaleString('zh-CN',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})}}
...
function loadGame(k){const r=localStorage.getItem(k);if(!r)return false;try{const d=JSON.parse(r);RESOURCES.gold=d.resources.gold;RESOURCES.food=d.resources.food;RESOURCES.wood=d.resources.wood;RESOURCES.stone=d.resources.stone;RESOURCES.water=d.resources.water||20;health=d.health!=null?d.health:80;research=d.research||0;officials=d.officials||0;population=d.population;popMax=d.popMax;satisfaction=d.satisfaction;villageLevel=d.villageLevel;day=d.day;gameTime=d.gameTime;buildings=[];const loadSz=getGridSize(villageLevel);GRID_COLS=loadSz.cols;GRID_ROWS=loadSz.rows;gridData.length=0;for(let i=0;i<GRID_COLS*GRID_ROWS;i++)gridData.push(null);for(const b of d.buildings){const o={type:b.type,workers:b.workers,level:b.level,guards:b.guards||0,scholars:b.scholars||0,index:b.index};gridData[b.index]=o;buildings.push(o)}for(const b of buildings){const bt=BT[b.type];if((b.workers||0)===0&&bt.maxW>0&&bt.prod&&Object.keys(bt.prod).length&&getIdlePop()>=1)b.workers=1}stats={...DEFAULT_STATS,...(d.stats||{})};unlockedAchievements=Array.isArray(d.unlockedAchievements)?d.unlockedAchievements.slice():[];gameWon=!!d.gameWon;for(const[k,v]of Object.entries(BT)){if(v.minLv<=villageLevel)v.unlocked=true}if(!Array.isArray(d.unlockedAchievements)){stats.maxPop=Math.max(stats.maxPop,population);silentSyncAchv()}initGrid();renderAll();updateUI();addLog(`读档成功（第${day}天）`,'good');return true}catch(e){addLog('读档失败：存档损坏','bad');return false}}
